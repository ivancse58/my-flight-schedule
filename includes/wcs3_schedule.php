...
function wcs3_render_day_table( $day ) {
    ...
    if ( $day_data ) {
        ...
        $output .= '<div class="wcs3-no-classes"><p>' . __( 'No Flight Schedule', 'wcs3' ) . '</p></div>';
    }
...
//    $wcs3_settings = wcs3_load_settings();
    ...
    if ( !empty( $r ) ) {
        foreach ( $r as $entry ) {
            $results[] = array(
//                ivan start
                'depart_airport_id' => get_post( $entry->depart_airport_id )->post_title,
                'arrive_airport_id' => get_post( $entry->arrive_airport_id )->post_title,
